...
def search_test_cases():
    ...
    if request.method == 'GET':
        ...
        rows = 1000
        if type == 'unattach':
            caseList = test_case_manage.test_case_manage().show_test_cases_unattach(id, conditionList, valueList,
                                                                                    fieldlist, rows)
        else:
    ...
    if request.method == 'GET':
        ...
        if id == '':
            ...
            if type == "" or type == 'test_suite':
                ...
                valueList.append(id)
                caseList = test_batch_manage.test_batch_manage().show_test_batch(conditionList, valueList, fieldlist,
                                                                                 rows)
        ...
        for j in range(len(ipList)):
            if ipList[j] == '':
                result0 = test_batch_manage.test_batch_manage().batch_new_testcase(test_suite_id, idrows,
                                                                                   browser_type_list=browser_list)
            else:
...
def general_test_batch():
    ...
    if request.method == 'POST':
        ...
        if run_type == '' or name == '':
            ...
            test_suite_manage.test_suite_manage().update_test_suite(id, ['name', 'run_type', 'description'],
...
def view_test_suite_screenshot():
    ...
    if request.method == 'GET':
        ...
        if len(imgUrl) == 0:
            return render_template('uitest/view_test_suite_screenshot.html', imgTitle='no screenshot!',
                                   imgCnt=len(imgUrl), id=id, test_batch_id=test_batch_id, type=type)
        else:
...
def test_keywords():
    ...
    if request.method == 'GET':
        ...
        fieldlist = []
        rows = 1000
        caseList = test_keyword_manage.test_keyword_manage().show_test_keywords(conditionList, valueList, fieldlist,
                                                                                rows)
        log.log().logger.info(caseList)
        data = caseList
        data1 = jsonify({'total': len(data), 'rows': data[int(offset):int(offset) + int(limit)]})
...
def save_new_test_keyword():
    ...
    result0 = test_keyword_manage.test_keyword_manage().new_test_keyword(name, paraCount, description, template,
                                                                         example)
...
def edit_test_keyword():
    ...
    if request.method == 'POST':
        ...
        template = viewutil.getInfoAttribute(info, 'template')
        example = viewutil.getInfoAttribute(info, 'example')
        result = test_keyword_manage.test_keyword_manage().update_test_keyword(id,
                                                                               ["keyword", "paraCount", "description",
                                                                                "template", "example"],
                                                                               [name, paraCount, description, template,
                                                                                example])
        return redirect('/testkeywords')
